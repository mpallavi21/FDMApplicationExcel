...
function CheckFDMDevicePropertyOption(DeviceName) {        
  ...
  try {
    ...
    aqObject.CheckProperty(Aliases.HCMClient.ClientMainWindow.MdiClient.CDeviceHomePage.panelBase.panelForDerivedForms.Panel.MagicTabControlEx.EntryPointTabPage.CDeviceScreen.m_pnlDeviceScreen.TabControl.EntryPointsTabPage.ElementHost.HwndSource_AdornerDecorator.AdornerDecorator.ScrollViewer.HyperlinkFdmDeviceProperties, "WPFControlText", cmpEqual, "FDM Device properties");
...
function LogUserAction(DeviceName) {
  Log.AppendFolder("LogUserAction - Workflow to Log and Verify User Action");
  ...
  try {
    ...
    ClickOnlineViewTab();
    clickOnNetworkViewTab();
    Log.Message("Navigated to Online and Network View tabs.");

    // Step 2: Access tree view and log user action
    ...
    Log.Message("Right-clicked device '" + DeviceName + "' and selected 'Log User Action'.");

    // Step 3: Fill in Description and Reason
    ...
    Log.Message("Entered description: " + AuditDescription);

    let reasonBox = auditTrailUserAction.ReasonTextBox.TextBoxArea;
    reasonBox.Click(39, 32);
    reasonBox.Keys("Log User Action");
    Log.Message("Entered reason: Log User Action");

    // Step 4: Confirm action
    OCR.Recognize(auditTrailUserAction.OKButton).BlockByText("OK").Click();
    Log.Message("Confirmed user action via OK button.");

    // Step 5: View Audit Trail and verify entry
    treeView.ClickItemR(DeviceName);
    treeView.StripPopupMenu.Click("View Audit Trail");

    let isLogged = FindValueInDescriptionColumn(AuditDescription);
    Log.Message("Audit entry verification result: " + isLogged);

    // Step 6: Cleanup
    CloseWindow();
    Log.Checkpoint("✅ LogUserAction passed: User action logged and verified successfully.");
  } catch (error) {
    Log.Error("❌ LogUserAction failed: " + error.message);
    status = "Fail";
  } finally {
    WriteResult("LogUserAction " + status, status, "Pass");
    Log.PopLogFolder();
  }
}
...
//    // === Step 5: Confirm device options ===
    ...
    Aliases.HCMClient.ManualDtm_PackageSelector.WinFormsObject("btnCancel").Click()
    Aliases.HCMClient.DeviceOptionsDlg.WinFormsObject("buttonCancel").Click()
...
function LoadDevice(Device) {
  ...
  WriteResult("LoadDevice " + status, status, "Pass");
  Log.PopLogFolder();
}
...
function SaveHistory() {
  ...
  try {
    ...
    WriteResult("Save history SaveHistory " + status, status, "Pass");
    Log.PopLogFolder();
  }
}
...
function GetDeviceStateView(DeviceName) {     
  Log.AppendFolder("GetDeviceStateView - Device Selection and State Capture Workflow");
  ...
  try {
    ...
    Log.Message("Device state information retrieved successfully.");

    // Step 4: Close configuration window and confirm
    CloseWindow();
    clickOnConfirmFDMButton();
    Log.Message("Configuration window closed and confirmed.");

    Log.Checkpoint("✅ GetDeviceStateView passed: Device state captured successfully.");
  } catch (error) {
    Log.Error("❌ GetDeviceStateView failed: " + error.message);
    status = "Fail";
  } finally {
    WriteResult("GetDeviceStateView " + status, status, "Pass");
    Log.PopLogFolder();
  }
}
...
function ReadAndWriteDeviceProperties(DeviceName, ChangeDeviceUsage, ChangeCalibrationstatus, ChangeDeviceHealth) {
  Log.AppendFolder("ReadAndWriteDeviceProperties - Workflow to Configure Device Properties");
  ...
  try {
    ...
    clickOnDevice(DeviceName);
    Log.Message("Selected target device: " + DeviceName);

    // Step 2: Open device properties via OCR hyperlink
    Aliases.HCMClient.ClientMainWindow.MdiClient.CDeviceHomePage.panelBase.panelForDerivedForms.Panel
      .MagicTabControlEx.EntryPointTabPage.CDeviceScreen.m_pnlDeviceScreen.TabControl.EntryPointsTabPage
      .ElementHost.HwndSource_AdornerDecorator.AdornerDecorator.ScrollViewer.HyperlinkFdmDeviceProperties.Click();
    Log.Message("Opened device properties via OCR hyperlink.");
    ...
    Log.Message("Device properties updated: Usage=" + ChangeDeviceUsage + ", Calibration=" + ChangeCalibrationstatus + ", Health=" + ChangeDeviceHealth);
    ...
    Log.Checkpoint("✅ ReadAndWriteDeviceProperties passed: Device properties configured successfully.");
  } catch (error) {
    Log.Error("❌ ReadAndWriteDeviceProperties failed: " + error.message);
    status = "Fail";
  } finally {
    WriteResult("ReadAndWriteDeviceProperties " + status, status, "Pass");
    Log.PopLogFolder();
  }
}
...
function auditTrailViaMenuTab() {
  Log.AppendFolder("auditTrailViaMenuTab - Workflow to Access Audit Trail via Menu Tab");

  let status = "Pass";

  try {
    // Step 1: Access main menu and click 'View'
    let HCMClient = Aliases.HCMClient;
    let frmHCMClientMain = HCMClient.ClientMainWindow;
    let smartMenuBar = frmHCMClientMain.mainMenu;

    OCR.Recognize(smartMenuBar).BlockByText("View").Click();
    Log.Message("Clicked 'View' in main menu.");

    // Step 2: Select submenu option
    HCMClient.DropDownForm.SubSmartControl.Click(48, 7);
    Log.Message("Selected submenu option under 'View'.");

    // Step 3: Navigate to connected tab
    ...
    Log.Message("Navigated to connected tab in Online View.");

    // Step 4: Access Audit Trail panel and click buttons
    let panel = frmHCMClientMain.MdiClient.AuditTrailView.panelBase.panelForDerivedForms.panel4;
    panel.panel2.groupBox1.ViewButton.Click(33, 6);
    Log.Message("Clicked 'View' button in Audit Trail panel.");

    panel.panel1.panel6.panel9.button5.Click(30, 18);
    Log.Message("Clicked secondary action button in Audit Trail panel.");

    // Step 5: Confirm action in FDM Configuration popup
    HCMClient.dlgFDMConfiguration.btnYes.ClickButton();
    Log.Message("Confirmed action in FDM Configuration popup.");

    Log.Checkpoint("✅ auditTrailViaMenuTab passed: Audit Trail accessed and confirmed successfully.");
  } catch (error) {
    Log.Error("❌ auditTrailViaMenuTab failed: " + error.message);
    status = "Fail";
  } finally {
...
function advanceConfiguration(TagName,PVLRV,PVURV) {
  Log.AppendFolder("advanceConfiguration - Workflow to Perform Advanced Configuration");

  let status = "Pass";

  try {
    // Step 1: Locate and click Advanced Configuration hyperlink
    let advanceConfigurationBtn = Aliases.HCMClient.ClientMainWindow.MdiClient.CDeviceHomePage.panelBase.panelForDerivedForms.Panel
      .MagicTabControlEx.EntryPointTabPage.CDeviceScreen.m_pnlDeviceScreen.TabControl.EntryPointsTabPage
      .ElementHost.HwndSource_AdornerDecorator.AdornerDecorator.ScrollViewer.HyperlinkAdvancedConfiguration;

    advanceConfigurationBtn.WaitProperty("Exists", true, 10000);

    if (advanceConfigurationBtn.Exists) {
      advanceConfigurationBtn.Click();
      Log.Message("Clicked Advanced Configuration hyperlink.");

      let tabControl = Aliases.HCMClient.ClientMainWindow.MdiClient.CDeviceHomePage.panelBase.panelForDerivedForms.Panel
        .MagicTabControlEx.EntryPointTabPage.CDeviceScreen.m_pnlDeviceScreen.TabControl;

      let AdvanceConfigurationPage = tabControl.FindChild("Name", `WinFormsObject(" Advanced Configuration ")`, 50);

      if (AdvanceConfigurationPage.Exists) {
        let HCMClient = Aliases.HCMClient;
        let adornerDecorator = tabControl.zAdvanced_Configuration_.ElementHost.HwndSource_AdornerDecorator.AdornerDecorator;
        let tableItemView = adornerDecorator.Control.TableItemView;

        // Expand first section
        if (!tableItemView.Expander.IsChecked) tableItemView.Expander.Click();

        // Configure first textbox
        let tableItemView2 = tableItemView.TableItemView;
        tableItemView2.Click(597, 11);
        let customTextBox = tableItemView2.CustomTextBox;
        customTextBox.Click(67, 11);
        customTextBox.Keys("[BS][BS][BS][BS][BS][BS][BS][BS][BS][BS]"+PVLRV);

        // Configure second textbox
        tableItemView2 = tableItemView.TableItemView2;
        tableItemView2.Click(491, 19);
        customTextBox = tableItemView2.CustomTextBox;
        customTextBox.Click(55, 8);
        customTextBox.Keys("[BS][BS][BS][BS][BS][BS][BS][BS][BS][BS]"+PVURV);

        // Expand nested sections
        tableItemView2 = tableItemView.TableItemView3;
        if (!tableItemView2.Expander.IsChecked) tableItemView2.Expander.Click();

        tableItemView = tableItemView2.TableItemView;
        if (!tableItemView.Expander.IsChecked) tableItemView.Expander.Click();

        // Configure third textbox
        tableItemView2 = tableItemView.TableItemView;
        tableItemView2.Click(587, 11);
        customTextBox = tableItemView2.CustomTextBox;
        customTextBox.Click(110, 11);
        customTextBox.Keys("[BS][BS][BS][BS][BS][BS][BS][BS][BS][BS]"+TagName);

        // Review and send configuration
        adornerDecorator.ButtonReviewSend.ClickButton();
        Log.Message("Clicked Review & Send.");

        let previewDialog = HCMClient.HwndSource_PreviewDialog.PreviewDialog;
        let button = previewDialog.btn_Send;
        aqObject.CheckProperty(button, "Enabled", cmpEqual, true);
        button.ClickButton();
        Log.Message("Clicked Send in Preview dialog.");

        let grid = previewDialog.Grid;
        aqObject.CheckProperty(grid.ProgressView, "Enabled", cmpEqual, true);
        grid.Grid.Grid.Rectangle.Click(5, 17);
        Log.Message("Verified progress view and clicked rectangle.");

        tabControl.InertButton.Click(3, 4);
        Log.Message("Clicked Inert button to finalize configuration.");

        Log.Checkpoint("✅ advanceConfiguration passed: Advanced Configuration completed successfully.");
      }
    } else {
      Log.Warning("⚠️ Advanced Configuration hyperlink not found.");
      status = "Fail";
    }
  } catch (error) {
    Log.Error("❌ advanceConfiguration failed: " + error.message);
    status = "Fail";
  } finally {
    WriteResult("advanceConfiguration " + status, status, "Pass");
    Log.PopLogFolder();
  }
}
...
function UnLoadDevice() {
  Log.AppendFolder("UnLoadDevice - Workflow to Unload Device");

  let status = "Pass";

  try {
    // Step 1: Close the device window
    status = CloseWindow();
    Log.Message("Closed device window successfully.");

    // Step 2: Confirm unload action
    clickOnConfirmFDMButton();
    Log.Message("Confirmed unload action via FDM button.");

    Log.Checkpoint("✅ UnLoadDevice passed: Device unloaded successfully.");
  } catch (error) {
    Log.Error("❌ UnLoadDevice failed: " + error.message);
    status = "Fail";
  } finally {
    WriteResult("UnLoadDevice " + status, status, "Pass");
    Log.PopLogFolder();
  }
}
